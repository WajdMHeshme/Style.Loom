...
/*
  ...
  - Same UI as before but Add to Cart now:
    1) dispatch(addToCart(...)).unwrap() to detect success/failure
    2) show SuccessMessage popup on success with product name
    3) keep existing favorite behavior
...
const ProductComponent: React.FC<ProductProps> = ({
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // The SuccessMessage component supports autoClose by default.
      // If you want to do something after the popup closes, pass a callback to onClose below.
    } catch (err) {
      // handle error (replace with toast if you prefer)
