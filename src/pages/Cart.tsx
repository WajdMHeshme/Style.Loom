...
// مصفوفة البداية (تستخدم فقط كـ initial state)
...
export default function Cart() {
  // الآن الحالة تُخزَّن فقط في الذاكرة (array عادي) وليس في localStorage
      ...
      <div className="border-2 border-dashed border-neutral-700 rounded-2xl p-6 max-w-5xl mx-auto">
        {cart.length > 0 ? (
          cart.map((item) => (
            ...
            >
              ...
              <button className="text-red-500 text-4xl" onClick={() => removeFromCart(item.id)}>
                <GoX />
              </button>
            </div>
          ))
        ...
        {cart.length > 0 && (
          <div className="flex justify-between items-center mt-6 flex-wrap gap-4">
            ...
            <button className="bg-yellow-700 text-white px-6 py-3 rounded-xl font-medium" onClick={() => setOpenPopUp(true)}>
              Checkout
            </button>
          </div>
        )}
      ...
      {openPopUp && (
        <div className="pt-[150px] fixed inset-0 bg-black flex items-center justify-center z-50">
          <div className="bg-neutral-900 rounded-2xl p-8 w-full max-w-lg relative">
            ...
            <button className="w-full bg-yellow-700 text-white py-3 rounded-xl font-medium" onClick={() => setOpenPopUp(false)}>
              Save
            </button>
          </div>
        </div>
